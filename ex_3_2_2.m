...
start = randi(img_len);
% 根据起始点生成长度
hid_len = randi(img_len-start);%定义并记录隐藏信息的长度
...
hidden(start: start + hid_len - 1) =  bitshift(bitshift(hidden(start: start + hid_len - 1), -1, 'int64'), 1, 'int64')+ hide_info;
...
extract_info = mod(extract_seq(start: start + hid_len - 1), 2);
%计算压缩比
